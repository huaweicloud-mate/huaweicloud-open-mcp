...
def is_default_region(region=None):
    return (region or current_region()) == DEFAULT_REGION


def raw_detail_path(region=None):
...
def raw_detail_partial_path(region=None):
    r = region or current_region()
    if r == DEFAULT_REGION:
...
def openapi_out_dir(region=None):
    r = region or current_region()
    if r == DEFAULT_REGION:
        return "data/openapi"
