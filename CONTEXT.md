...
**is_obs** — the routing predicate (`src/mcp_openapi/execute_obs.py`) deciding whether an operation goes down the OBS execution lane (`product == "OBS"`, host starting with `obs.` as fallback). The OBS lane — `ObsSigner` (HMAC-SHA1, `signer/obs.py`), dict→XML body serialization, path-style bucket addressing, XML `<Error>` parsing, and the `ObsHttpClient` transport adapter — is entirely self-contained behind this predicate; the generic SDK-HMAC-SHA256 lane is untouched. `service.execute_api` dispatches on `is_obs(product, doc)`.

**ObsRequest** — the value type `execute_obs` builds from user params: `(bucket, object_key, query, headers, body, content_type)`, naming one OBS operation's parsed request before signing/sending. `bucket_name` (declared `in: query` by API Explorer metadata) moves to path-style addressing; `object_key` is the sole path parameter; `Authorization`/`Date` header params are dropped (regenerated by the signer).