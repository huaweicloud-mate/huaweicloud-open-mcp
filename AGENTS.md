...
- execute_api 服务方言（`_DEFAULT_JSON_CT_PRODUCTS`，2026-09 起）：产品名单 `{MRS, LTS, GES, AAD, MSGSMS, WAF, UGO}` 的无 body 请求由 real lane 编排层自动补默认头 `Content-Type: application/json`（build_request 后 `setdefault`，显式传入不覆盖）——这些服务的头校验对缺 CT 的 bodyless 请求以 415/400 拒绝（三种错误形态：MRS/UGO/LTS 415——MRS 为 `cluster_infos` handler 族 `12000006`、LTS `LTS.0393`，GES `GES.0001`、AAD/MSGSMS/WAF 为 APIG orchestration 400），2026-09 全量探测实证（219 产品/397 探针矩阵：方言产品 +CT 后越过头校验，191 个产品无 CT 即 200）。加头对非方言端点无害：SDK-HMAC-SHA256 签名排除 content-type（`sign.signed_headers_list`），签名输出逐字节不变，官方 SDK 全局携带为既成先例；产品级（非路径级）为正确粒度（方言产品的非方言端点加 CT 无害）。Graduation trigger：方言规则超出「产品名单 + 通用默认头」形态（如按产品注入 X-Environment-Id/Client-Request-Id 等用户上下文头——探测发现的 AOS/CAE/CloudTable/DataArtsStudio/IdentityCenterPortalAPI 等缺头属此类，明确不修）时，升级为独立方言模块（OBS lane 先例）。
