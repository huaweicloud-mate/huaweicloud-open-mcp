...
| `configs/` | safety policy 示例（含 server 规则）、`openapi-hints.example.json` 自定义提示注入示例、`metadata-corrections.json` 元数据纠偏条目（doc-verified 逐 API 事实修正，RDS:StartupInstance 起步；FunctionGraph:CreateEvent 畸形 pattern 指针 replace）、tag 中文→英文翻译映射、`mcp-server-catalog.example.json` 本地目录；发布时经 hatch force-include 打进 wheel（`huaweicloud_open_mcp/configs/` 包数据），运行时经 `common.paths.config_path` 解析（仓库根优先 → 安装态包内资源回退）；`--hints`/`--deprecated-index`/`--metadata-corrections` 額外支持裸文件名（经 `resolve_config_arg`：显式路径 > 仓库根 configs > 包内 configs） | — | — |
