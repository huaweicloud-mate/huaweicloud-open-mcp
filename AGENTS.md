...
| `src/safety/` | safety policy 解析与匹配（PolicyRule 含 kind=product/server；支持 `product:apiPattern=` 与 `server:serverId[:toolPattern]=` 两种规则前缀）+ `policy_store.py` PolicyStore（策略状态层：文件↔内存双向同步、mtime 热重载、原子落盘、三档 scope——permanent 落盘 / temporary 内存+TTL 惰性剪枝 / session 内存 overlay，生效规则=overlay 前置++文件规则整体 first-match，供 manage_policy 与运行时热更新共用） | — | — |
...
| S2b | `safety/policy_store.py` PolicyStore（rules 热重载 / add_rule(scope, ttl_seconds) / remove_rule 跨层 / text / list_rules，文件↔内存双向同步 + 三档 scope overlay） | 单测：tmp 文件注入 + 内容哈希 stat 替身 + 注入时钟（time_fn）；服务层「拒→add→同实例立即放行」 | 直接回读磁盘原始内容 + `parse_policy` 交叉验证 |
...
| E2 | elicitation 端到端：两模式 server 装配（execute_api/call_server_tool/connect/manage_policy 的 PolicyConsent 接线 + `build_*_app(elicit_mode)`） | 真 mcp SDK client + InMemoryTransport 内存回环（脚本化 elicitation callback；`common.http.fetch_json` monkeypatch 封死元数据网络） | tmp policy 文件回读 + 结构化结果断言（granted_rule/会话内生效文件不变；显式 scope=permanent 才落盘） |
...
- safety policy 三档 scope（`PolicyStore.add_rule(scope=..., ttl_seconds=...)`，2026-09 起）：`permanent` 永久（写策略文件，跨重启）/ `temporary` 临时（内存 overlay + TTL，ttl_seconds 缺省 3600s，取规则时惰性剪枝）/ `session` 会话内（内存 overlay，server 进程存活期，**缺省档**，重启即失、无需回收）。生效规则 = overlay（插入序）++ 文件规则，整体行序 first-match——overlay allow 穿透文件具体 deny 与 `*=deny` 兜底（与落盘插位语义一致），overlay deny 可临时收紧；overlay 内复用「插到首个遮蔽 deny 之前」不变量；`remove` 跨层匹配（先会话/临时后文件）并回报 `scope`，不接受 scope/ttl_seconds 参数；`manage_policy` list 返回结构化 rules（line/scope/expires_in，评估序）+ 文件全文；ttl_seconds 仅与 temporary 组合且须为正；未配置 policy 文件时三档全拒（红线不变）；TTL 时钟经 time_fn 注入（默认 time.monotonic）。**默认档由「add 即落盘」改为 session 属破坏性变更**：需跨重启持久须显式 `scope="permanent"`。
