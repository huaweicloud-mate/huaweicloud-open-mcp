...
| `configs/` | safety policy 示例（含 server 规则）、`openapi-hints.example.json` 自定义提示注入示例、`metadata-corrections.json` 元数据纠偏条目（doc-verified 逐 API 事实修正，RDS:StartupInstance 起步）、tag 中文→英文翻译映射、`mcp-server-catalog.example.json` 本地目录；发布时经 hatch force-include 打进 wheel（`huaweicloud_open_mcp/configs/` 包数据），运行时经 `common.paths.config_path` 解析（仓库根优先 → 安装态包内资源回退）；`--hints`/`--deprecated-index`/`--metadata-corrections` 額外支持裸文件名（经 `resolve_config_arg`：显式路径 > 仓库根 configs > 包内 configs） | — | — |
...
- 元数据纠偏（`apie/metadata_corrections.py`，2026-09 起）：API Explorer 元数据偶有与官方帮助文档不一致的过期文案（首例：RDS StartupInstance 的 x-constraint 残留「该接口仅支持PostgreSQL引擎」，官方文档 rds_05_0026 接口约束无引擎限制——功能上线时 PostgreSQL 先行后扩展至其它引擎，文档已更新而 Explorer 未同步）。纠偏条目为 doc-verified 的逐 API 精确键事实修正，shipped `configs/metadata-corrections.json` 随 wheel 分发，`--metadata-corrections`/env 部署自助（裸文件名同 `--hints` 口径；缺省加载 shipped 文件缺失静默空、`off`/空串显式禁用、显式路径缺失 fail-fast、非法配置启动快速失败仿 hints 严格校验）。schema：键 `PRODUCT:API` casefold；条目 `{evidence?, doc_url?, verified?, patches}`，patch 二选一 `drop`（行级子串删行，全删光信封置 None 保键形/doc 移除键）或 `replace`（整字段无条件落位含缺失新建）；字段白名单 v1 仅 `x-constraint`。**接缝在 presentation 层（hints 同位）**：`ToolService.get_api` 对 `format_api_detail` 新鲜信封 copy-on-write 纠偏（`correct_api_result`），缓存 doc 恒不改写；离线管道 convert 阶段经 `correct_doc` 组合同一 patch 核心（shipped 文件默认加载）；`catalog`/`live_fallback`/`convert_api` 接口零改动。纠偏生效面 = get_api 信封 + 离线产物；execute 校验不读约束文本无涉；api-docs CLI 保持读原始数据（调试工具）。红线：逐 API 精确键禁止通用模式删除（RDS「数据库代理(PostgreSQL)」等 tag 的「仅支持PostgreSQL」是真约束，泛化误杀）；drop/replace 幂等，上游修复后自动 no-op 条目可退场；`MetadataCorrections.empty()`/None 恒 no-op。台账：load 时 INFO 列条目（启动一次），调用时静默；evidence/doc_url/verified 仅存纠偏文件不进结果信封。新增纠偏流程：官方帮助文档核实独立真值 → 纠偏文件加条目（通常仅 drop——真值多为「官方无此限制」，不发明替代文案）→ 发版或部署侧挂 flag 自救。
