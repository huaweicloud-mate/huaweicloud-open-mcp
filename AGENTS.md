...
| `src/mcp_openapi/` | openapi 模式（metadata/execute 纯函数 + `gate.py` 产品门栓 + `hints.py` 自定义提示注入 + `entity_search.py` BM25 检索引擎（S16a：tantivy RAM 索引 + 手动 CJK 2-gram/驼峰 token 化 + 字段 boost/DisMax/单产品补召回 + provenance） + `entity_graph.py` 实体图谱检索（S16b：EntityGraph 值对象——parse 严格校验 + 身份信号（alias 嵌句/整别名/contains/gram 四档）+ 广度先验 + twin 归并 + load 三分支） + service 编排层 + server 装配；配置/客户端工厂注入；元数据加载委托 apie.catalog；service 层 8 个工具方法经 `_audited` 装饰器统一写审计事件，`build_audit_event` 定义对 verifier 的已发布 payload 契约 `tool/input/ok`） | — | — |
