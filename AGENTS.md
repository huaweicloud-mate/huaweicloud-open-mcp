...
本项目实现华为云 Open MCP server：本地通用网关（Core 模式，默认 stdio、`--transport http` 提供 Streamable HTTP 单进程多会话），openapi 模式用 8 个核心工具、discover 模式用 8 个发现连接工具、data 模式用 2 个本地数据工具（DataFusion 只读 SQL 分析与转换落盘），触达华为云全量 OpenAPI / 云端 MCP server / 本地数据，供 AI 客户端（Claude Code / opencode / Cursor 等）通过自然语言查询、调用、转换与分析华为云服务。同类产品参考阿里云 OpenAPI MCP Server（Core 模式）与 AWS Labs MCP。
...
│                 原子写盘+覆盖门+小预览；不经 policy，无 manage_policy——
...
- **安全**：`execute_api` 过 safety policy（API 级 allowlist/denylist 模式匹配）；未配置 policy 时拒绝所有执行；凭证必须是最小权限 IAM 用户的 AK/SK。授权模型 = safety policy（唯一授权接缝）+ AK/SK 硬边界（产品门栓 Gate 已于 2026-09 移除：产品级白名单与 policy 职责重叠，且 discover 模式本就无门栓先例）。
...
| `src/apie/` | APIE 管道实现（fetch/split/convert/merge/organize/refresh/api_docs + http 抓取助手 + mock 端点客户端）+ `explorer.py` 注册域数据源深模块（v4/products / v2/apis / v3/apis/detail；方言归一：productshort→product_short、method 小写、信封键改写；1055 去 region 兜底、ApiNotFoundError；counts v4 派生）+ `memory_store.py` 纯内存缓存 + `catalog.py` 远端优先功能接口（缓存命中直接返回，未命中实时拉取）+ 帮助中心补全管线（`help_docs.py` 解析/匹配/差集/hints 生成纯函数核心 + `fetch_help_docs.py` 种子探测+BFS 抓取编排 + `build_help_hints.py` 差集派生落盘）+ 实体图谱构建管线（`build_entity_graph.py` 确定性构建纯函数 + `entity_extract.py` LLM 构建期语义抽取编排：transport/sleep 注入、三道闸校验、指纹增量、curated 优先合并）+ `metadata_corrections.py` 元数据纠偏（shipped 配置驱动的逐 API 精确键事实修正，应用面唯一——`doc_compose.py` 组合根 convert+correct 同一核心（ADR-0001），live_fallback 缓存写入前 in-place + 离线 correct_doc 共同委托，doc 级 JSON Pointer patch v1 前缀白名单 /definitions/） | — | — |
...
| `src/mcp_openapi/execute_obs.py` | OBS 执行 lane：`is_obs` 路由谓词 + `OBJECT_DATA_APIS`/`is_object_data_api` 对象数据面名单（真实模式恒走 presign 单口径）+ 桶寻址（带桶 virtual-hosted/无桶端点根）+ consumes 三态 body 分流（json/xml/octet-stream）+ 开关型子资源自动补全 + `Content-MD5` 自动计算 + dict→XML 序列化（根元素经转换管线 `x-xml-root` 保留）+ XML `<Error>` 解析 + 响应体分类单点直调 `parse_body`（bytes 交给 `normalize_response` 统一占位+落盘）+ `_presign` 预签发编排（零字节搬运）+ `ObsHttpClient` 适配器 + 编排 | — | — |
| `src/common/auth/` | 凭证加载（env/profile，project_id 自动获取） | — | — |
| `src/safety/` | safety policy 解析与匹配（PolicyRule 含 kind=product/server 与 once 标记；支持 `product:apiPattern=` 与 `server:serverId[:toolPattern]=` 两种规则前缀）+ `policy_store.py` PolicyStore（策略状态层：文件↔内存双向同步、mtime 热重载、原子落盘、四档 scope——permanent 落盘 / temporary 内存+TTL 惰性剪枝 / session 内存 overlay（code-agent 会话档，stdio 下等价进程存活期）/ once 一次性用后即焚，`authorize`/`authorize_server` dispatch 前原子授权门，生效规则=overlay 前置++文件规则整体 first-match，供 manage_policy 与运行时热更新共用）+ `manage_policy_ops`（2026-09 起：manage_policy 工具信封两模式单一实现——action 归一/list/add/remove 分派/结果信封/日志，store=None 拒绝统一 NOT_CONFIGURED_REASON；openapi/discover service 各一行委托） | — | — |
| `src/mcp_openapi/` | openapi 模式（metadata/execute 纯函数 + `execute.py` 执行接缝（CONTEXT.md A：`ApiExecutor` 双 adapter——`RealApiExecutor`（build_request→CT→basePath→X-Project-Id→`SignedClient` 签名发送）与 `MockApiExecutor`（剥控制键→mock 端点，passthrough）+ `RequestRefusal`（真实 lane 两类拒绝，execute_api 捕获转信封）+ `validate_params`/`build_request`/`normalize_response` 纯函数；service 层 lane 决策单点，`is_obs` 恒一次求值） + `hints.py` 自定义提示注入 + `entity_search.py` BM25 检索引擎（S16a：tantivy RAM 索引 + 手动 CJK 2-gram/驼峰 token 化 + 字段 boost/DisMax/单产品补召回 + provenance） + `entity_graph.py` 实体图谱检索（S16b：EntityGraph 值对象——parse 严格校验 + 身份信号（alias 嵌句/整别名/contains/gram 四档）+ 广度先验 + twin 归并 + load 三分支） + service 编排层 + server 装配；配置/客户端工厂注入；元数据加载委托 apie.catalog；service 层 8 个工具方法经 `_audited` 装饰器统一写审计事件，`build_audit_event` 定义对 verifier 的已发布 payload 契约 `tool/input/ok`） | — | — |
...
| `src/common/elicit.py` | PolicyConsent：safety policy 变更的 elicitation 交互语义（offer_grant 拒绝提议授予（粗规则存在时五选一 GrantChoiceConfirm：api=最小 / api_session=最小 session 档 / product=产品级 session 档 / readonly=产品级只读规则集（offer.readonly_rules 数据驱动，无数据即不可达）/ none） / gate_change 变更确认门 / gated_manage_policy 两模式共享的 manage_policy 工具体（确认门+service 委派，docstring 留各 registrar） / fallback_hint 未问询路径拒绝兜底指引 / parse_elicit_mode / PolicyChangeConfirm+GrantChoiceConfirm 表单 schema / ElicitFn adapter 契约 + ctx_elicit_fn MCP Context 归一化 adapter（confirm/choice 独立归一）） | — | — |
| `src/common/paths.py` | 项目根路径解析（统一 project_root） | — | — |
| `src/common/sessions.py` | 会话身份 ambient 契约（ADR-0003，S17：layer 1 零内部依赖、零 mcp import；公共面恰 2 名 `current_session_key`/`SessionScopeMiddleware`——SDK ServerMiddleware adapter 是生产唯一 setter，头解析与 set/reset 为 internal seams；解析梯子 header→私有 conn→None 单函数收拢，S17 spike 钉死 2.0.0 两腿均可达；stdio/InMemory 恒 None） | — | — |
| `src/common/deployment.py` | 部署旋钮归一（2026-09 起）：全部 `HUAWEICLOUD_MCP_*` env 名常量唯一定义处 + `Deployment` 值对象（跨模式共享旋钮 + env 映射随行）+ `resolve_deployment(args, env)`（getattr 安全，mock 布尔/policy/audit 回退链单点） | — | — |
| `src/common/logconf.py` | 日志配置：文件为主（logs/{program}.log 轮转）+ stderr WARNING+ 兜底 | — | — |
| `src/huaweicloud_open_mcp/` | server 入口包：`deployment.py` 装配单入口（build_app external seam，见 ADR-0002）+ `cli.py` CLI（argv 解析 + 日志配置 + build_app + run）+ `__main__.py`（`python -m`）+ `__version__`（发布版本单一真值源，hatch dynamic version 读取） | — | — |
...
| `configs/` | safety policy 示例（含 server 规则）、`openapi-hints.example.json` 自定义提示注入示例、`help-docs-hints.json` 缺省 hints 快照 + `help-docs-hints-curation.json` curated 提示种子（helphints 再生合并，curated 永不丢）、`metadata-corrections.json` 元数据纠偏条目（doc-verified 逐 API 事实修正，RDS:StartupInstance 起步；FunctionGraph:CreateEvent 畸形 pattern 指针 replace）、tag 中文→英文翻译映射、`mcp-server-catalog.example.json` 本地目录；发布时经 hatch force-include 打进 wheel（`huaweicloud_open_mcp/configs/` 包数据），运行时经 `common.paths.config_path` 解析（仓库根优先 → 安装态包内资源回退）；`--hints`/`--deprecated-index`/`--metadata-corrections` 額外支持裸文件名（经 `resolve_config_arg`：显式路径 > 仓库根 configs > 包内 configs） | — | — |
...
第4层  src/huaweicloud_open_mcp/  入口包，按 --mode（可逗号组合）延迟 import 对应 server
           │                      + deployment.py（装配单入口 build_app：共享 store/sink 构造注入
           │                        + manage_policy 去重 + instructions 合并；composite.py 已废）
           │
...
第2层  src/safety/policy       src/apie/                  │
         + policy_store                                  │
          （纯函数零依赖；        ├ catalog → live_fallback → convert_openapi2
           ...
           │                        │
第1层  src/common/            （types / http / paths / sessions / logconf / audit / auth/credentials，均零内部依赖）
```
...
| `mcp_openapi/` | → `apie`（catalog/metadata/mock/memory_store）+ `safety` + `common` + 外部 `tantivy`（核心依赖，entity_search 独占 import）；`service`→`execute`+`execute_obs`+`signer.client`+`signer.obs`；`entity_graph`→`entity_search`；`signer.client`→`signer.sign`；`execute_obs`→`execute`+`signer.obs`+`common` |
...
| `huaweicloud_open_mcp` | → `common.logconf` + `common.deployment`（Deployment 值对象/env 常量单点/resolve_deployment）+ `deployment.build_app`（external seam：延迟 import 三 mode server/config + `safety.policy_store`；per-mode `build_X_config`/`build_X_app` 降为 internal seams） |
...
- **入口包化**：入口在 `src/huaweicloud_open_mcp/` 包（顶层 `main.py` 已废——site-packages 通用名冲突）；单模式与混装同一路径经 `deployment.build_app`（2026-09 起装配单入口，external seam 唯一——共享 PolicyStore/AuditSink 构造注入、合并 instructions、manage_policy 全局注册一次归属优先级 openapi > discover；composite.py 删除，见 docs/adr/0002）；cli 仅 argv 解析+日志配置+build_app+run；per-mode `build_X_config`/`build_X_app` 为 internal seams（dep=None 原口供配置解析测试直调）。
...
- **环境变量**：遵循华为云 SDK 惯例——`HUAWEICLOUD_SDK_AK`/`HUAWEICLOUD_SDK_SK`/`HUAWEICLOUD_SDK_SECURITY_TOKEN`/`HUAWEICLOUD_SDK_PROJECT_ID`；MCP 自身配置用 `HUAWEICLOUD_MCP_*` 前缀（如 `HUAWEICLOUD_MCP_MOCK`、`HUAWEICLOUD_MCP_POLICY_FILE`、`HUAWEICLOUD_MCP_OPENAPI_HINTS`（自定义提示注入配置）、`HUAWEICLOUD_MCP_AUDIT_FILE`（审计 NDJSON 落盘路径）、`HUAWEICLOUD_MCP_MOCK_PASSTHROUGH`（mock 模式转发业务参数）、`HUAWEICLOUD_MCP_ELICIT`（policy 变更 elicitation 确认模式 auto/required/off，默认 off）、`HUAWEICLOUD_MCP_SPILL_DIR`（openapi 超限落盘目录，空串/off 禁用，缺省系统临时目录 hwc-mcp-spill）、`HUAWEICLOUD_MCP_DEPRECATED_INDEX`（openapi 废弃接口索引文件）、`HUAWEICLOUD_MCP_DEPRECATED_MODE`（list_apis 废弃处理模式 annotate/hide/off）、`HUAWEICLOUD_MCP_ENTITY_INDEX`（openapi 实体图谱索引，缺省加载 configs/entity-index.json，缺失静默禁用）、`HUAWEICLOUD_MCP_AUTH_DEMOTE`（认证头 required 元数据归一开关 on/off，默认 on）、`HUAWEICLOUD_MCP_AUTH_DEMOTE_PASS`（认证头归一豁免名单 PRODUCT:API 精确/PRODUCT 产品级，逗号分隔）、`HUAWEICLOUD_MCP_METADATA_CORRECTIONS`（openapi 元数据纠偏配置文件）、`HUAWEICLOUD_MCP_TRANSPORT`（传输层 stdio/http，`streamable-http` 别名归一）、`HUAWEICLOUD_MCP_HTTP_HOST`/`HUAWEICLOUD_MCP_HTTP_PORT`/`HUAWEICLOUD_MCP_HTTP_PATH`（HTTP 监听三元组，默认 127.0.0.1:8000/mcp））；构建期 LLM 抽取（api-refresh graph --llm）用独立前缀 `ENTITY_LLM_BASE_URL`/`ENTITY_LLM_API_KEY`/`ENTITY_LLM_MODEL`（通用 OpenAI-compatible，缺一即跳过抽取）。discover 模式新增：`HUAWEICLOUD_MCP_MODE`（运行模式，可逗号组合：openapi/discover/data，如 `openapi,data`）、`HUAWEICLOUD_MCP_SERVER_CATALOG`（目录文件路径）、`HUAWEICLOUD_MCP_SESSION_IDLE_TIMEOUT`、`HUAWEICLOUD_MCP_MAX_SESSIONS`。
...
```bash
uv sync                                  # 安装依赖（含 dev，dev 组含 datafusion/sqlparse；核心依赖含 tantivy——search_apis BM25 引擎）
...
- `release.yml`（tag `v*` 推送 → 正式 PyPI + GitHub Release 附 dist；workflow_dispatch 手动选 `target=test|prod`，默认 test）：版本守卫（tag 触发时强制 `v{__version__} == tag`）→ 构建 + twine check → `uv publish --index <testpypi|pypi>`（token 取 secrets `UV_PUBLISH_TOKEN_TEST`/`UV_PUBLISH_TOKEN_PROD`，与 `scripts/publish` 同名约定；job 声明 `environment: PUBLISH`，两 secret 配置在该 GitHub Environment 下）→ tag 触发时 `softprops/action-gh-release`。CI 内无交互确认门（tag 推送即显式发布动作）。
...
uv run api-refresh graph --llm --llm-force  # 绕过指纹台账全量重抽（prompt/闸门调整后用）
...
uv run huaweicloud-open-mcp --transport http   # Streamable HTTP 多会话（127.0.0.1:8000/mcp；session 档 policy 按 MCP 连接会话隔离）
...
uv run huaweicloud-open-mcp --auth-demote off        # 认证头 required 元数据归一开关（默认 on；环境变量 HUAWEICLOUD_MCP_AUTH_DEMOTE）
uv run huaweicloud-open-mcp --auth-demote-pass "RDS:ListVolumeInfo,DDS:*"  # 归一豁免名单（PRODUCT:API 精确/PRODUCT 产品级；环境变量 HUAWEICLOUD_MCP_AUTH_DEMOTE_PASS）
uv run huaweicloud-open-mcp --metadata-corrections configs/metadata-corrections.json  # 元数据纠偏（缺省加载同路径 shipped 文件；off 显式禁用；环境变量 HUAWEICLOUD_MCP_METADATA_CORRECTIONS）
...
| 接缝 | 内容 | 测试方式 | 独立真值 |
| --- | --- | --- | --- |
| S0 | `common.paths` 配置资源解析：`config_path(name)`（仓库根 configs/ 优先 → 安装态包内 configs 资源回退，wheel 由 hatch force-include 映射）+ `resolve_config_arg(value)`（启动参数路径：存在的显式路径原样 > 仓库根 configs > 包内 configs，全缺失 FileNotFoundError 列全候选）+ `common.optconf`（2026-09 起：可选文件配置加载 discipline 单一实现——`load_opt_file(arg, parse, off, default_name)` 三分支（None→缺省档 config_path 解析缺失静默 off / off+空串大小写不敏感→off 值 / 显式路径 resolve_config_arg fail-fast）+ `is_off` 哨兵判定单点（spill 复用修大小写漂移）；hints/corrections/entity_index/deprecated 四 loader 一行委托，deprecated 补 off 支持；枚举型解析器（elicit/modes/deprecated_mode/auth-demote）语义真差异不入） | 纯函数单测（monkeypatch project_root/config_path + 真实仓库布局 cwd 无关断言） | tmp 目录文件系统状态 + 包内资源路径结构 |
...
| S3 | 各工具业务函数 `mcp_openapi.service` / `apie.metadata`（manage_policy 编排委托 `safety.policy_store.manage_policy_ops`（2026-09 起：信封语义两模式单一实现下沉 safety，service 仅薄委托 + 模式特有 docstring）；`_audited` 审计挂钩：8 工具统一记 `{ts, tool, input, ok}` NDJSON，input 为显式入参快照，异常路径记 ok=False；execute_api 一次性授权消费点（校验失败/接口未找到不烧）+ manage_policy 误路由引导） | 单测，迷你样本 fixture；audit sink 以 tmp 文件/内存替身注入 | 自建迷你 OpenAPI 片段（仿 apis fixtures 设计，不依赖真实 raw/ data/）+ 回读磁盘原始 NDJSON |
| S4 | `execute_api` 执行接缝（CONTEXT.md A，2026-09 起）：`ApiExecutor` 双 adapter（`RealApiExecutor`=build_request 路径填充/参数切分+默认 CT+basePath 前缀+X-Project-Id→`SignedClient`（原 ApiExecutor Protocol 更名）签名发送；`MockApiExecutor`=剥 `_status_code/_number`→mock 端点，passthrough 经 `apie.mock` 编码）+ `RequestRefusal`（缺必填路径参数/缺 host，execute_api 捕获转 `{ok:false,reason}`，mock 恒不抛）+ `execute_api(location, product, api, region, params, *, executor, spill)` 编排（规范化+信封包装）+ service 层 lane 决策单点（C6：`is_obs` 恒一次求值，审计 mode 归 lane/executor）+ mock passthrough（`--mock-passthrough` 开启时标量→query、body→POST JSON、`_` 控制键剥离，编码对齐 real 模式 HttpClient；默认关保持 API Explorer 契约）+ `parse_body` 二进制检测（不可无损 UTF-8 解码/含 NUL→bytes 原样，合法 UTF-8 路径逐字节不变） | 集成测试直连 mock 端点 + 本地回环 CaptureServer + 单元层 urllib 打桩注入错误（429/4xx/5xx）+ test_executors（adapter 矩阵：路径填充/basePath/CT/X-Project-Id/RequestRefusal/控制键剥离）+ parse_body 纯函数矩阵（字节字面量：损坏字节/NUL/全 ASCII/JSON/文本/空体） | mock 端点返回（HTTP 恒 200；`status_code` 非 200 返回空 body）+ 回环 stub 请求台账 |
| S5 | APIE 管道各阶段转换（含认证头 required 降级：`AuthDemotePolicy`/`demote_auth_headers`/`parse_auth_demote_policy`——默认归一/大小写变体/doc-global·path·op 三处/豁免精确+产品级/禁用/幂等矩阵；参数 `$ref` 解析与占位符补全：`_resolve_param_ref` 三值契约（解析 deepcopy 隔离/缺失容错/网关供给头 CT 丢弃·大小写变体）+ `_clean_params` 去重键恢复（多 ref 保序、inline+ref 同名去重、path 级 ref 解析）+ `_complete_path_params`（三层级声明幂等注入/project_id 凭证提示/无缺失不突变）+ convert_api 接线金标：真实 raw fixture `tests/fixtures/functiongraph_invoke_function_raw.json` 转换后 function_urn 恢复 + format_api_detail 可见） + `apie.memory_store` 内存缓存层（set/get/clear/LRU）+ `apie.explorer` 注册域数据源（方言归一矩阵：productshort→product_short/method 小写/信封键改写；1055 双兜底→ApiNotFoundError；分页 count+空批终止；counts 派生；fake fetcher 注入）+ `apie.catalog` 功能接口（内存缓存优先→远端回退决策，monkeypatch `apie.http.fetch_json`/`fetch_json_retry` 边界，detail 走 explorer 委托，auth_demote 线程断言）+ `apie.metadata_corrections` 元数据纠偏（parse 严格校验矩阵/三分支 load（None→shipped 静默空、off 显式禁用、路径 fail-fast）/`FieldPatch.apply` 字符串核心（drop 删行·全删光 None、replace 落位、幂等）correct_doc doc 级（全删光移除键、replace 新建、指针级每 doc 一次）/键 casefold/字段白名单 fail-fast/evidence 不泄漏）+ `apie.doc_compose` 组合根（ADR-0001：convert+correct 同一核心，live_fallback 缓存写入前 in-place + 离线 main() 共同委托，correct_doc_cow/correct_api_result 已删——缓存 doc 必已纠偏由构造保证） | 纯函数单测 + 迷你样本集成 + `@pytest.mark.e2e` 全量 | Swagger 2.0 schema 校验；monkeypatch 注入 HTTP 响应控制远端回退路径 |
| S6 | benchmark 纯函数（`benchmarks/cases.py` 加载校验（含可选 `fixture`/`labels`/`policy` 扩展字段）、`scorer.py` 分层评分 + `event_to_toolcall`（审计事件→trace 输入适配，双消费者：legacy runner 与 harbor verifier）、`report.py` 统计/基线对比、`trace.py` export/NDJSON 提取（CONTEXT.md B：`parse_export` → 单一类型化 `ExportResult`（usage/trace/recovered），截断 regex 恢复藏于本模块（recover 门控，export_session 仅末次尝试启用）；`extract_trace` 仅工具序列，answer 唯一真值源为 live NDJSON 流 `parse_run_output`，export 的 answer 覆写已删） + export JSON info 的 token 读取、`stub_server.py` 本地回环、`runner.py build_benchdir_config`（legacy 与 harbor agent 双适配器）、`harbor/exporter.py` render_task 纯核/export_dataset 薄壳、`harbor/conventions.py build_agent_opencode_config`、`harbor/task_templates/stub_server.py` fixture 引擎（resolve_response/append_ledger 纯核 + GET/POST 回环）） | 纯函数单测（trace 用 spike 实测格式的迷你 fixture；stub 用回环 HTTP；exporter 用 mini project_root + 金标字面量） | 手写字面量 + 独立构造的样例调用序列 |
...
| S9f | 预签发 URL（`signer.obs.url_string_to_sign` / `sign_obs_url` / `build_presign_base` + `execute_presign_api` 编排 + service 对象数据面自动 presign / 显式 `_presign` 分派与热更新协同；S9f-c：GetObject 签发前 HEAD 预检 `_presign_head_check`（复用 `ObsClient` port 注入，404 拒签/异常降级/`expected_size`+`expected_etag` 填充/`client=None` 回归红线）） | 纯函数单测 + service 注入凭证 | 官方《URL中携带签名》表4/5 结构原文 + openssl 口径金标 |
| S10 | `mcp_openapi/hints.py` 自定义提示注入（parse_hints/load_hints_file + Hints 值对象 product_notes/api_notes/combined_notes 合并策略）+ `ServiceConfig.hints` service 注入（list_products 条目级、get_product/list_apis 顶层、list_apis 条目级 API 级、get_api 顶层合并；拒绝路径与 get_api_examples/execute_api 恒不注入）+ server `build_instructions` 部署自定义指引段 + `--hints`/env 装配（load_hints_file 委托 common.optconf 三分支：None→缺省档 configs/help-docs-hints.json 缺失静默、off/空串→显式禁用、显式路径 fail-fast） | 纯函数单测（键归一化经 lookup 方法验证，不 peek 字段内部）+ service 注入 Hints 信封断言 + 装配断言（sealed_configs fixture 密封真实仓库） | 手写字面量 + off/缺省文件缺失时结果为 empty 的回归红线 |
| S11 | data 模式：`mcp_data/engine.py`（外部接缝 `run_query`（读）/`run_transform`（写）双 lane：表注册/只读守卫/执行/JSON-safe 规范化/双重截断 + 写 lane 原子落盘/覆盖门/产物回读；内部接缝纯函数 `assert_readonly_sql`/`json_safe`/`truncate_rows`/`_sniff_output_format`/`_count_written`；datafusion 惰性 import 未安装→`DataError` 安装指引）+ `mcp_data/service.py` DataService（audit 信封 + DataError→ToolError 翻译，engine 模块形注入 Protocol）+ `mcp_data/server.py` 装配（单 data 模式工具集恰 `{query_data, transform_data}`）+ `deployment.build_app` 混装（工具集并集/manage_policy 去重/instructions 合并/共享 store）+ `cli.parse_modes` 逗号多值 | engine 纯函数单测（手写字面量矩阵）+ 真 datafusion 集成（inline/csv/parquet/ndjson/join/聚合/三格式互转/覆盖门/原子落盘/COPY 拒绝，无网络）+ service 注入 engine 模块形替身 + InMemoryTransport e2e | 手写字面量 + Python 独立重算的聚合真值 + 产物文件独立回读 + monkeypatch `sys.modules["datafusion"]=None` 模拟未安装 |
| S12 | 响应落盘：`mcp_openapi/spill.py`（`spill_body` 层级 1 截断前全保真 + `guard_result` 层级 2 通用信封守卫 + `SpillConfig`/`parse_spill_config` 装配（off 哨兵经 common.optconf.is_off，大小写不敏感——2026-09 修复 "OFF" 被当目录路径）；内部接缝 `_spill_payload` 唯一落盘机制）+ execute 三 lane 接线（`normalize_response(resp, spill, stem)`，含 bytes 分支：占位 `binary/size/content_type/sha256` + `.bin` 无条件落盘、disk == wire 保真、非 2xx bytes 错误体占位+hex 短描述）+ service `_guarded` 装饰器（8 工具统一） + `_spill=false` 控制键剥离（guard 同步跳过）+ cli/server/composite 装配（`--spill-dir`/`HUAWEICLOUD_MCP_SPILL_DIR`、data_enabled 部署感知披露） | 纯函数单测（tmp_path 文件回读/原子性/唯一名/格式保真/best-effort 失败；guard 预算矩阵/占位替换/逐级收缩/refusal 与已落盘与已截断 no-op） + execute/service/obs 注入测试（超限文件独立回读、未配置回归红线、控制键不进 query、binary 占位+`.bin` sha512 独立回读、OBS lane 统一形状） |
...
| S16a | 实体检索引擎 `mcp_openapi/entity_search.py` TantivyEngine（build(apis, boosts, tie_breaker) RAM 索引 / search(terms, limit) / search_product 单产品补召回：schema（5 个 raw token 字段 + 6 个存储原始字段）/ token 化 doc-query 同规则（驼峰切分须用原始大小写）/ 字段 boost / 术语间 DisMax / 全局 top 归一化 / provenance (field, term) 溯源） | 纯函数单测（mini fixture 建索引/查询确定性/归一化界/驼峰与 CJK 双轨/boost 旋钮/provenance/tie-break）+ 真数据 smoke（build 耗时入档，无网络） | 行为锚定（排序/界/溯源），不复算 BM25 公式；真 tantivy 不 mock（in-process） |
| S16b | 实体图谱 facade `mcp_openapi/entity_graph.py`（parse 严格校验不变 + `EntityGraph.search_apis`：_terms 切分 / 身份信号（alias 嵌句 18 > 整别名 18 > contains 8（ASCII ≥3 字符门）> gram 6；product 12 / name 5 / category 1）/ 判别 tag bonus 2（严格子串）+ BM25 max ×12 + 广度先验 2.5·(log(1+n)/log(150))²（零证据不计）/ twin 归并 / matched_via 优先级排序 cap 3 / exclude_apis 机制参数 / limit clamp 与 -1 哨兵）+ `load_entity_index` 三分支（不变）+ 金评集门禁 `tests/eval_runner.py` | 单测：结构断言（limit/exclude/twin/证据 cap/parse/load）+ 行为锚定（排序与证据，分数断言仅身份档精确值）+ 金评集 45 例全绿（真 configs/entity-index.json） | tests/fixtures/entity_eval.json 人工标注独立真值（top1/top3/family 三档）；mini fixture 行为字面量 |
| S15c | service/server/cli 装配：`ToolService.search_apis`（空图谱拒绝、`_audited`/`_guarded` 接线、审计 input 快照 {query, limit, category}；废弃治理同 list_apis——hide→exclude_apis 机制参数排名前排除（twin 各成员按自己的索引精确过滤）/ annotate→`_annotate_search_deprecated` service 层 copy-on-write 标注 deprecated+replacement（已知边界：twin 归并行内孪生成员 api 按主产品索引查询可能欠标注，hide 无此问题）/ off 未配置恒现状）+ server 第 8 工具注册 + `--entity-index`/env 三分支装配 + instructions 第 0 步 + build_app 工具集并集（8+8+2=17，manage_policy 去重 10） | service 注入测试 + build_openapi_config 装配断言 + build_instructions 断言 | 手写字面量 + InMemoryTransport 工具集断言（test_composite） |
| S15e | LLM 构建期抽取 `apie/entity_extract.py`（`refresh_knowledge` 编排纯函数：roster/API 指纹增量（sha256，roster 变更→产品级重抽，产品 API 变更→该产品关键词重抽）、分批（产品 60/批、API 120/批）、四道闸校验（严格 JSON 解析容忍围栏 / 交叉验证 target∈产品集·alias≠中文名·长度 1-8·kw 1-12·每 API ≤5·去自环·note 截 60 / 域约束闸门——关键词/别名含他产品判别 ASCII token（≥3 字符、恰单产品认领，孪生共享不判别）丢弃 / 支配性闸门——别名被 >2 个非 target 产品别名包含则泛词丢弃 / 丢弃台账 WARNING）、批失败跳过且指纹不更新（可恢复）、sleep_fn 注入、force=True 绕过指纹全量重抽（--llm-force，prompt/闸门调整后用））+ `merge_knowledge` internal seam（世代语义：update 提供 key 才替换 llm 代，否则沿用旧 llm；curated 永不覆盖；api_keywords/fingerprints 按产品 overlay）+ `load_llm_env`/`make_openai_transport`（common.http post_json_retry 429 退避） | 纯函数单测（fake transport 罐头 + 记录器；校验/合并/指纹矩阵；输入不变性） | 手写罐头响应与期望 knowledge 字面量；指纹重复性（同输入同指纹） |
...
| S13c | `apie/help_docs.py` 匹配差集生成：`build_alias_index`（歧义丢弃）/`match_apis`（overrides→别名→api_name→summary 兜底，duplicate/ambiguous 台账）/`detail_descriptions`/`diff_completions`（仅差集阈值）/`build_hints`（S10 schema + round-trip）+ `build_help_hints.apply_curation_to_hints`（curated 提示合并内缝：再生不丢/同键 curated 优先/None 恒同对象/copy-on-write） | 纯函数单测 + `parse_hints` round-trip | 手写 mini index/records 字面量矩阵 |
| S13d | `apie/fetch_help_docs.py` + `apie/build_help_hints.py` 编排：种子探测+BFS（done 页用记录 links 不重取）+ 断点续传/failed 台账去重与清账/dead_seeds/limit 预算 + `build_completions` 三产物落盘回读（curation 参数接线，configs/help-docs-hints-curation.json 存在则合并） + HELP_STAGES 接入（refresh 默认范围不变） | 注入 fetch_fn/sleep_fn + tmp_path 文件回读 | 手写字面量 + 产物文件独立回读 |
...
| S14b | `mcp_openapi/deprecated.py` DeprecatedIndex（entry/names/empty + parse 严格校验；load 委托 common.optconf，None/off/空串→空索引（2026-09 起 off 支持——修复 "off" 被当路径 fail-fast），显式路径 fail-fast） | 纯函数单测 | 手写字面量矩阵 |
...
| E1 | `common/elicit.py` PolicyConsent（parse_elicit_mode / offer_grant（含 readonly 多规则 best-effort 授予）/ gate_change / fallback_hint / ctx_elicit_fn 归一化）+ `safety/policy.py` grant_rule/grant_server_rule/readonly_grant_rules | 纯函数单测，脚本化 ElicitFn + 记录型 grant 注入 | 手写字面量 + `parse_policy` 交叉验证规则文本 |
...
| S17 | `common/sessions.py` 会话身份 ambient 契约：解析梯子矩阵（header 优先/私有 conn 兜底/归 None/hostile 输入不抛）、middleware set/reset 异常安全、嵌套内层胜出、notification None 覆盖、stdio 形 ctx 绑 None | 纯函数单测（duck-typed fake ctx 字面量；spike 钉死 SDK 2.0.0 两腿均可达） | 手写 fake ctx 字面量 + 真回环全链归 S22 |
| S18 | PolicyStore 会话键控（ADR-0003 I1–I7）：互斥桶 first-match、strict fail-closed（session/temporary/once 拒绝、permanent 放行）、跨会话不可见（含 list_rules）、once 限授予会话焚毁、remove 当前桶→文件 + 「先前会话」提示、三重 GC（idle/age/cap LRU，注入时钟）、读路径零分配、空键归一、session_fn=None 全矩阵与 S2b 逐字一致 | 单测：注入 session_fn 替身交替会话 + tmp 文件 + 注入时钟（S2b 同法）；既有 test_policy_store.py 全量即回归红线 | 手写策略字面量 + 磁盘回读 + parse_policy 交叉验证 |
| S19 | transport 归一与 run 分派：`parse_transport` 别名/fail-fast、argv>env>默认三元组、端口/路径校验、裸 Namespace getattr 安全、`run_transport` 记录型 app 双分派（stdio 逐字节/http 参数） | 纯函数单测 + 记录型替身 | 手写字面量矩阵 |
| S20 | audit 会话归因：`_audit_write` 单点（键 None 恒无 session 字段=NDJSON 逐字节、非 None 顶层加键、input 快照纯净、异常路径同样归因）、`build_audit_event` 契约形状冻结（signature 恰 tool/input_args/result） | service 注入 MemorySink + `_bind_session` 内缝 | 手写字面量 + 既有 test_audit 逐字段等值断言 |
| S21 | build_app 会话装配：middleware 结构断言、HTTP 档 instructions 传输说明（stdio 无）、strict_sessions 按传输置位经真 InMemoryTransport 工具面（http 档 manage_policy add session 档结构化拒绝 / stdio 档放行 / http 档 permanent 放行）、**组合行为测试（复审 F4）**——真 middleware + 真 contextvar + 真 store 串全链（A 落桶 B 不可见、退出复位、无身份拒绝） | 装配断言 + InMemoryTransport + 组合链（零 mock，fake ctx 仅替代 SDK 边界） | 手写字面量 + 磁盘回读 |
| S22 | HTTP 多会话 e2e（真 mcp SDK client × build_app × 回环 uvicorn）：双客户端并发授予互不可见、重连即新会话（remove 未命中附提示）、permanent 跨会话、审计 NDJSON 会话归因（≥2 会话、input 无 session 键）、modern 单交换无身份 manage_policy 结构化拒绝（I2 协议可达性）、/healthz、execute 拒绝预网络短路 | 回环 HTTP 集成（uvicorn 线程 + `streamable_http_app()`，无外网无凭证） | 真客户端行为对照 + NDJSON 回读 + 策略文件回读 |

分层与纪律：

- **单元测试**（`tests/test_signer.py`、`tests/test_obs_signer.py`、`tests/test_obs_execute.py`、`tests/test_safety.py`、`tests/test_tools_*.py`、`tests/test_apie_*.py`、`tests/test_service.py`、`tests/test_client.py`、`tests/test_bench_*.py`、`tests/test_mcp_discover_*.py`、`tests/test_data_engine.py`、`tests/test_data_service.py`）：纯函数，不联网、不碰真实数据；service 层用 monkeypatch HTTP 注入 + 客户端工厂注入。
...
- 二进制响应字节保真（2026-09 起）：`parse_body` 分类判据为载荷内在属性（与 Content-Type 无关）——不可无损 UTF-8 解码或含 NUL → 返回原始 `bytes`（不透明二进制），合法 UTF-8 路径行为逐字节不变（JSON/text 回归红线）。bytes body 三 lane（real/mock/OBS）共用 `normalize_response` 渲染：body 恒为占位体 `{"binary": true, "size": N, "content_type": ct, "sha256": <wire 字节摘要>, "note": ...}`（sha256 供 agent `sha256sum` 独立校验；content_type 自 headers 大小写不敏感提取），完整字节**无条件**落盘 `.bin`（无体积门槛——不落盘即整段丢失；spill 未启用/失败时 note 明示数据不可得，不再输出乱码）；非 2xx bytes 错误体同样占位+落盘，`error_msg` 为 `binary body (N bytes): 前 16 字节 hex`。**保真不变量（disk == wire）**：任何分类下 spill 落盘内容与线上字节逐位一致——不可解码/NUL 分支 bytes 直写；全 ASCII 二进制误分类为 str 时 decode/encode 字节恒等仍无损。原 OBS lane 专属二进制占位（`render_response_body`/`_TEXTUAL_CT_MARKS`）已删除，判据单点收拢于 `parse_body`（OBS 客户端 adapter 直调），占位形状与 real/mock lane 统一。典型场景：FG ExportFunction zip、镜像层、导出文件类 API；大对象仍应走 presign 直连（OBS 对象数据面单口径不变）。
- execute_api OpenAPI 元数据 schema 校验（默认开启，`execute.validate_params` 纯函数，presign 后/三 lane 分流前共享）：只校验文档声明了的参数（未声明宽容透传、`_` 控制键跳过）；query 必填/类型/枚举**严格**（integer/number/boolean 不接受字符串形式，bool 混入数值显式排除，`type(v) is int` 防 bool 陷阱）；header 只查必填（协议即字符串；认证头 x-auth-token/x-security-token/authorization 匹配**大小写不敏感**——元数据 casing 混乱（小写 869 处/混合 19 处），精确匹配曾对 698 个 API op 误报「缺少必填 x-auth-token」）；body 用 jsonschema Draft4 + doc.definitions resolver 校验；失败返回 `{"ok": false, "reason": 可操作描述（参数/期望/实际值）}` 供 agent 自纠。路径参数校验归 real lane（build_request 内守卫——mock URL 不含 path，mock 环境无凭证时路径填充必然缺失，提前校验会系统性误杀）；OBS lane 跳过（XML body 不适用）；校验失败经 `_audited` 记 ok=False。`_path_param_values` 是路径参数知识的内部接缝（validator/builder 共享，build_request 因此降为纯 mechanism）。
- 认证头 required 元数据归一（2026-09 起，`apie/convert_openapi2.py`）：本网关认证恒由 AK/SK 签名层供给（SDK-HMAC-SHA256 生成 Authorization + X-Security-Token），元数据「认证头必填」在内部契约里恒为假——实测语料 casing 混乱（x-auth-token 小写 869 处/混合 19 处；required=true 的认证头参数 doc-global 2,864 处、op 级 7,229 处），`convert_api` 在 finalize 后统一把 `in=header` 且名字 casefold ∈ {x-auth-token, x-security-token, authorization} 的参数 required 降级为 false（幂等 in-place，覆盖 doc-global/path 级/op 级三处；豁免命中整 doc 保持原样）——get_api 展示与 execute 校验同源一致，agent 不再被迫构造 token。开关 `--auth-demote on|off`（默认 on，env `HUAWEICLOUD_MCP_AUTH_DEMOTE`）；豁免名单 `--auth-demote-pass "RDS:ListVolumeInfo,DDS:*"`（env `HUAWEICLOUD_MCP_AUTH_DEMOTE_PASS`，PRODUCT:API 精确 / PRODUCT 或 PRODUCT:* 产品级，casefold 归一，非法条目启动快速失败仿 hints 严格校验）——名单内 API 元数据保持原样，**仅影响元数据归一，不影响校验层**（校验层无条件跳过认证头必填；豁免 API 缺 token 放行到服务端拿真实 401，服务端为真值兜底）。策略为启动期常量：doc 转换后随内存缓存固化，同进程内变更策略不回溯已缓存 doc。配置线程：`ServiceConfig.auth_demote → ToolService.load_api_doc → catalog.find_api_doc → LiveFallback → convert_api`；离线管道（data/openapi）与 api-docs CLI 走默认策略（统一降级、无豁免，运行时 live 转换持有 per-deployment 豁免）。典型坏例：RDS::ListVolumeInfo（GET /v3/{project_id}/instances/{instance_id}/volumes）元数据 x-auth-token required:true 为假，且该路由在华为云服务端本身损坏——坏签名打 /instances 进认证层拿 APIGW.0301 401，打 volumes 认证前 415（Tomcat HTML），POST 报 APIGW.0101（仅注册 GET），实证 APIG 该路由后端映射配置损坏、任何客户端不可达（2026-09 探针闭环），客户端侧无解、仅此存档。
- 参数 `$ref` 解析与路径占位符声明补全（2026-09 起，`apie/convert_openapi2.py`，无部署开关）：API Explorer 元数据的 op/path 级参数大量用 `$ref: #/parameters/X` 引用 doc-global 声明，而 `finalize` 旧去重键 `name|in` 对 ref 形参数恒为 `None|None`（`oas2_parameter` 对无 `in` 键的 ref 原样透传），每个 op 仅第一个 ref 幸存——全语料实测 4,562 个 op 丢失 2+ 个参数声明（3,035 个 path 参数声明被吞，涉 31 产品；FunctionGraph:InvokeFunction 的 `function_urn` 即此 root cause，raw 元数据本身完整）。修复三件套（纯元数据事实修复，幂等）：
  - **ref 解析**（`finalize` 内缝 `_resolve_param_ref` + `_clean_params`，op 级与 path 级参数列表共用）：去重前把 ref 展开为 doc-global 目标深拷贝（deepcopy 防 demote in-place 互染），随后过 `oas2_parameter` 归一；指向缺失/非 dict 的 ref 原样保留容错（validate_params 天然跳过无 name 参数）；去重键恢复 name|in 语义（容错保留的 ref 按 `$ref` 指针去重）。认证头 ref 解析后由既有 demote_auth_headers 自动降级，零新增代码。
  - **网关供给头 ref 丢弃**（`_GATEWAY_SUPPLIED_HEADERS`，现仅 content-type，casefold）：ref 形 CT 解析期直接丢弃——real lane 对全部请求 setdefault CT，required=true 的 CT 解析 inline 化会造成系统性 validate 误拒（validate_params 只豁免认证头）；ref 形 CT 维持历史语义（去重时代被吞、不可见不校验）。**inline 形 CT（语料 1,415 op 现存 validate 误拒：不显式传 CT 参数即被拒）本次不动，单独立项**。极少数「第一个 ref 恰是 CT」的 op 从 get_api 展示 CT 变为不展示（减少误导）。
  - **占位符补全**（`_complete_path_params`，convert_api 内 demote 后调用，幂等 in-place）：与 ref 折叠正交的第二类缺口——raw 元数据真缺声明的路径占位符（全语料 1,132 op：非 project_id 1,023 op/25 产品如 Config/RMS/IAM/GA/CC/ER，仅缺 project_id 109 op）注入合成声明（in=path/required=true/type=string，description 标注「元数据未声明，网关自动补全」，project_id 附「可由凭证自动填充」）；声明检查覆盖 doc-global（in=path）/path 级/op 级三层，任一层已声明即不注入。运行时语义不变：build_request 路径替换从 path 模板提取占位符（`_path_param_values`），从不依赖声明。修复后全语料 convert_api 输出未声明占位符归零（验证口径：raw 正确 ref 感知扫描 1,132 → 0）。
- 元数据纠偏（`apie/metadata_corrections.py`，2026-09 起）：API Explorer 元数据偶有与官方帮助文档不一致的过期文案（首例：RDS StartupInstance 的 x-constraint 残留「该接口仅支持PostgreSQL引擎」，官方文档 rds_05_0026 接口约束无引擎限制——功能上线时 PostgreSQL 先行后扩展至其它引擎，文档已更新而 Explorer 未同步）。纠偏条目为 doc-verified 的逐 API 精确键事实修正，shipped `configs/metadata-corrections.json` 随 wheel 分发，`--metadata-corrections`/env 部署自助（裸文件名同 `--hints` 口径；缺省加载 shipped 文件缺失静默空、`off`/空串显式禁用、显式路径缺失 fail-fast、非法配置启动快速失败仿 hints 严格校验）。schema：键 `PRODUCT:API` casefold；条目 `{evidence?, doc_url?, verified?, patches}`，patch 二选一 `drop`（行级子串删行，全删光信封置 None 保键形/doc 移除键）或 `replace`（整字段无条件落位含缺失新建）；字段白名单 v1 仅 `x-constraint`；**doc 级指针 patch（2026-09 起）**：patches 键以 `/` 开头视为对 doc 根的 JSON Pointer（RFC 6901，~0/~1 归一），v1 前缀白名单 `/definitions/`（schema 事实修正类），其余前缀/空段快速失败；指针 patch 二选一 `replace`（非空字符串，叶值落位，父链存在时含缺失新建）/`drop`（布尔 true 删叶键——与 op 级行级列表形区分，语义随目标类而变）；首例 FunctionGraph:CreateEvent：Explorer pattern `'$[a-zA-Z][a-zA-Z0-9-_]*'` 以 $ 开头在 re.search 语义下永不匹配，execute_api body 校验拒绝一切合法 name，官方真值 `^[a-zA-Z]([a-zA-Z0-9_-]*[a-zA-Z0-9])?$`（全语料 17,963 接口仅此一例「永不匹配」畸形，其余形似怪异 pattern 功能正常不泛化误修）。**应用面唯一（ADR-0001，2026-09 起 3→1 收敛）**：`apie/doc_compose.py` `compose_doc(raw, *, product, api, auth_demote, corrections)`（convert + correct 同一核心）——live_fallback（运行时，**缓存写入前 in-place**，「缓存 doc 必已纠偏且不再改写」由构造保证）与离线 convert main() 共同委托；corrections 配置沿 auth_demote 先例穿线程（ServiceConfig → load_api_doc → catalog.find_api_doc → LiveFallback，启动期常量随首次转换固化进缓存）；`correct_doc_cow`/`correct_api_result` 已删除（对已纠偏 doc 恒 no-op 的假想 seam）；service 对 doc 纯读（get_api 信封、execute 校验口径天然同源）；api-docs CLI 保持读原始数据（调试工具）。纠偏生效面 = 缓存 doc（进而 get_api 信封 + execute 校验口径）+ 离线产物。红线：逐 API 精确键禁止通用模式删除（RDS「数据库代理(PostgreSQL)」等 tag 的「仅支持PostgreSQL」是真约束，泛化误杀）；drop/replace 幂等，上游修复后自动 no-op 条目可退场（replace 落位值为官方文档文案，上游采纳同一文案即 no-op）；`MetadataCorrections.empty()`/None 恒 no-op。台账：load 时 INFO 列条目（启动一次，op 级字段 + 指针路径），调用时静默；evidence/doc_url/verified 仅存纠偏文件不进结果信封。新增纠偏流程：官方帮助文档核实独立真值 → 纠偏文件加条目（通常仅 drop——真值多为「官方无此限制」，不发明替代文案；schema 事实类用指针 replace 落官方值）→ 发版或部署侧挂 flag 自救。
- execute_api 默认 Content-Type（2026-09 起，real lane 全局口径）：所有请求（含无 body 的 GET/DELETE 等读方法）在 build_request 后 `setdefault` 补默认头 `Content-Type: application/json`——官方 SDK 全局携带 CT 且从不注入 body 为既成先例；SDK-HMAC-SHA256 签名排除 content-type（`sign.signed_headers_list`），加头对签名输出逐字节不变；显式传入不覆盖（setdefault 语义），恒不注入空 JSON 体（P9 `body:{}` 手工绕过把头与体混同，注入无独立真值支撑且改变请求语义，明确不采纳）。演进：原「方言产品名单 `{MRS, LTS, GES, AAD, MSGSMS, WAF, UGO}` + 写方法集 `{POST, PUT, PATCH}`」两层口径（2026-09 全量探测实证：MRS/UGO/LTS 415——MRS 为 `cluster_infos` handler 族 `12000006`、LTS `LTS.0393`，GES `GES.0001`、AAD/MSGSMS/WAF 为 APIG orchestration 400；RDS StartupInstance 无 body POST 415 实证方言为端点级现象，产品名单粒度结构性不可表达）因新方言产品持续出现（打地鼠、名单追不上）收敛为全局默认；收敛前对「非方言 GET 带 CT」格做真实凭证冒烟实证 200（ECS/VPC/RDS/IAM 各一只读 GET + MRS 回归），原 GET 红线「非方言不带」撤销。Graduation trigger：规则超出「通用默认头」形态（如按产品注入 X-Environment-Id/Client-Request-Id 等用户上下文头——探测发现的 AOS/CAE/CloudTable/DataArtsStudio/IdentityCenterPortalAPI 等缺头属此类，明确不修；或按端点差异化头值）时，升级为独立方言模块（OBS lane 先例）。
- execute_api basePath 前缀（2026-09 起）：real lane 在 `build_request` 后把转换后 doc 的标准 Swagger `basePath` 作为请求路径前缀（非空且非 `/` 才拼、尾部 `/` 归一，签名按最终路径自动重签）——API Explorer `/v3/apis/detail` 载荷的 snake 字段 `base_path` 经 `convert_openapi2.fix_2_doc` 保留为 doc `basePath`，9 产品 488 接口非根（APIG `/v2`+`/v1.0`、VAS/AOM `/v2`、CodeArtsInspector `/v3`、KooPhone `/v1`、OrgID `/orgid/openapi/v1`、IoTEdge/CloudRTC/CampusGo），17,963 接口全量统计 path 均不含前缀（双前缀风险 0），缺省 `/` 与缺失路径行为逐字节不变（回归红线）；get_api 展示 path 与 Explorer parity 不拼前缀；mock lane（URL 不含真实 path）与 OBS lane（独立编址）不适用。实证：APIG 全部专享版接口修复前 404 `APIGW.0101`（共享网关不识别无版本路径），修复后 `ListInstancesV2` 200。
...
- safety policy 四档 scope（`PolicyStore.add_rule(scope=..., ttl_seconds=...)`，2026-09 起）：`permanent` 永久（写策略文件，跨重启）/ `temporary` 临时（内存 overlay + TTL，ttl_seconds 缺省 3600s，取规则时惰性剪枝）/ `session` 会话内（内存 overlay，**缺省档**，重启即失、无需回收）/ `once` 一次性（内存 overlay，**仅放行下一次执行，用后即焚**，重启即失；与 ttl_seconds 互斥）。**`session` 档语义（2026-09 起）= 当前 code agent 会话**（AI 客户端与 gateway 的一次连接会话）：stdio 单进程单会话下实现为进程级内存 overlay（重启即失），「进程存活期」与 code-agent 会话等价；**Streamable HTTP 部署（ADR-0003，2026-09 起）= 当前 MCP 连接会话**——内存 overlay 按会话键分桶（`session_fn` 构造注入 + ambient contextvar），连接互不可见、重连即新会话、桶三重惰性 GC（idle 1800s/年龄 86400s/cap 4096 LRU），`strict_sessions=true`（HTTP 装配置位）时无会话键的 session/temporary/once 写入结构化拒绝（I2 fail-closed）。生效规则 = 当前会话桶 overlay（插入序）++ 文件规则，整体行序 first-match——overlay allow 穿透文件具体 deny 与 `*=deny` 兜底（与落盘插位语义一致），overlay deny 可临时收紧；overlay 内复用「插到首个遮蔽 deny 之前」不变量；`remove` 跨层匹配（当前会话桶 → 文件，不触他会话桶；HTTP 会话未命中附「先前会话」提示）并回报 `scope`，不接受 scope/ttl_seconds 参数；`manage_policy` list 返回结构化 rules（line/scope/expires_in，评估序=调用者可见集）+ 文件全文（text() 恒文件全文，不泄会话规则）；ttl_seconds 仅与 temporary 组合且须为正；未配置 policy 文件时全档全拒（红线不变）；TTL 时钟经 time_fn 注入（默认 time.monotonic）。**默认档由「add 即落盘」改为 session 属破坏性变更**：需跨重启持久须显式 `scope="permanent"`。
...
- safety policy elicitation 确认（`common/elicit.py` PolicyConsent + `--elicitation`/`HUAWEICLOUD_MCP_ELICIT`，**默认 off**，2026-09 起）：默认关闭时不发任何 MCP elicitation——`manage_policy` add/remove 直接生效热更新（缺省会话内档、不落盘；`gate_change` 恒放行）；policy 拒绝返回但 reason 追加 `fallback_hint` 兜底指引（prompt 约定：引导 LLM 经对话/交互式问询——如 question 工具——向用户确认后经 manage_policy 授予，选项语义与五选一表单一致）+ 审计 NDJSON（唯一硬保障）。开启后才启用协议级确认门——`manage_policy` add/remove 前服务端经 elicitation 向用户确认（`gate_change`，confirm 才生效）；`execute_api`/`call_server_tool`/`connect_mcp_server` 被 policy 拒绝时服务端经 elicitation 提议授予（`offer_grant`，accept → 授予并在 denial 上附 `granted_rule` + 引导重试；**openapi execute_api 为五选一 GrantChoiceConfirm 表单——api=最小规则 `scope="once"`（一次用户确认仅放行下一次执行，用后即焚）/ api_session=最小规则 `scope="session"`（会话内持续放行该单 API，重启即失；单功能粒度，与 product 选项的广度区分）/ product=产品级规则（`product:*=allow`，`scope="session"` 会话内放行该产品全部目标，重启即失）/ **readonly=产品级只读规则集（2026-09 起，S-B）**：`safety.policy.readonly_grant_rules(product)` 构造 `P:*List*/*Show*/*Get*/*Query*=allow` 四条（语法知识归 safety，offer 携带 `readonly_rules` 数据、PolicyConsent 数据驱动），逐条 `scope="session"` 授予——**best-effort 无回滚**（session 档内存 overlay 无文件 I/O、规则文本构造期校验，add 失败近乎不可达；部分失败如实报告已授予/失败明细，只读+会话内部分授予无害；不引入 revoke seam），`granted_rule` 以 `;` 连接；浏览类任务首选，同会话第二次问询建议直接推荐 session/readonly / none=不授予；discover call_tool 保持四选一（DenialOffer 不挂 `readonly_rules` → readonly 选项数据缺席即不可达，误选按 none 处理）；coarse_rule 缺失时退化为单一确认；discover connect 无产品级选项（call 级规则匹配不到 connect 检查），保持会话内单一授予**；choice→scope 映射内聚于 PolicyConsent（api→minimal_scope 由调用点注入，api_session/product/readonly→恒 session）；grant 失败原因附入 reason）。**兜底指引口径（fallback_hint）**：凡 policy 拒绝且存在可授予 offer、但未发生任何 elicitation 的路径（off 档 / auto 与 required 下客户端不支持）——denial reason 追加可操作指引（经交互式问询确认后 manage_policy 授予，coarse 存在时并列五选项、readonly 仅在 offer.readonly_rules 存在时出现）；decline/cancel/refuse 与授予路径不加（用户已表态或已入流程）。模式语义：off 从不 elicit，拒绝路径 reason 附兜底指引；auto 尝试 elicit，客户端不支持（`ctx.elicit` 失败）→ 降级为 prompt 兜底记 WARNING（reason 附兜底指引）；required 不支持 → manage_policy 返回可操作 reason（fail-closed），拒绝路径不提议（reason 附兜底指引）。规则文本由 `safety/policy.py` grant_rule/grant_server_rule/readonly_grant_rules 构造（parse_policy 交叉验证）；denial_reason 与 policy 复查不一致（非 policy 拒绝源）/未配置 policy/非 denial 结果一律不提议；decline/cancel 不写审计 NDJSON（仅 WARNING，`_audited` 契约不变；accept 的授予经 manage_policy 正常入审计）。**默认 off 的原因**：① 各 code agent 对 MCP elicitation 支持参差（2026-09 实测——Claude Code/Cursor/VS Code 支持；opencode 1.x 不声明 elicitation capability（client 仅声明 roots），v2 分支已实现（上游 PR #35064）但未发版且 headless 有挂起风险（#36076）；Codex 部分支持且有 bug；Gemini CLI 不支持）；② auto 的降级放行使确认门语义随客户端能力漂移——同一部署在不同 agent 下「一个有门、一个没门」；③ 默认关闭保证跨客户端行为一致、可预测，符合项目 fail-safe 默认哲学（未配置 policy 全拒）。需要交互确认门的部署显式传 `--elicitation auto/required`（或 `HUAWEICLOUD_MCP_ELICIT`）。
- 产品门栓（`Gate`）：已于 2026-09 移除（删除测试接缝 S8）。openapi 授权模型收敛为 safety policy（API 级，唯一授权接缝）+ AK/SK 硬边界；产品发现面（list_products/search_apis/get_product/list_apis/get_api/get_api_examples）不再有产品级拦截，越权执行由 policy 拦（execute_api 未配置全拒，红线不变）。
...
- 自定义提示注入（`Hints`，2026-09 起）：openapi 模式部署侧提示（`--hints`/`HUAWEICLOUD_MCP_OPENAPI_HINTS`，启动加载、无热更新、仅 openapi 模式；路径支持裸文件名，经 `resolve_config_arg` 按显式路径 > 仓库根 configs > 包内 configs 解析，`load_hints_file` 内部接线）；配置 schema `{"instructions", "products": {PRODUCT: "notes" | {"notes", "apis": {API: text}}}}`，产品键归一化 upper（productshort 惯例）、API 键归一化 lower（大小写不敏感，对齐 live_fallback），严格校验非法配置启动快速失败；注入语义为**附加式**——官方 `summary`/`description` 永不替换，提示以独立 `hints` 字段伴随返回（`get_api` 顶层为产品+API 合并文案，合并策略内聚 `Hints.combined_notes`：产品在前、空段跳过、换行连接、双空不加字段）；注入点仅 6 处——server instructions 追加「部署自定义指引」段（全局 text 非空时）、`list_products` 条目级、`get_product`/`list_apis` 顶层产品级、`list_apis` 当前页条目级 API 级、`get_api` 顶层合并；policy 拒绝路径与 `get_api_examples`/`execute_api` 恒不注入（拒绝不泄漏越权提示、审计契约不动）；**缺省档（2026-09 起）**：`--hints` 与 env 均未配置时自动加载 `configs/help-docs-hints.json`（仓库根副本优先 → 包内副本，缺失**静默** `Hints.empty()`——隐式缺省不 fail-fast；`load_hints_file` 三分支：None→缺省档、空串/`off`（strip+大小写不敏感，对齐 spill idiom）→显式禁用、其余显式路径缺失 fail-fast，JSON 非法恒 fail-fast），`off`/空串为显式禁用逃生门；塑造发生在 service 层（`apie/metadata.py` 纯函数不感知部署配置），全部 copy-on-write 不改写 metadata 返回对象；顶层可选布尔键 `api_notes_in_list_apis`（S13f，缺省 true=现状）为 false 时 `list_apis` 条目级 API 提示被抑制（顶层产品级与 `get_api` 合并提示不受影响），帮助中心补全生成文件显式置 false。
...
- 实体关联图谱与 `search_apis`（S15，2026-09 起）：渐进式工作流第 0 步——用户意图未指明产品/API 时的跨产品检索。**构建期两段**：①确定性基座（`apie/build_entity_graph.py` 纯函数：节点=huawei_products 收录且有 API 的产品（218/310，死端排除）、twin=同名中文产品两两互指（主产品运行时按 link 非空 > API 数 > 字典序判定，ECS/HCSECS 16 组）、attributive=归属（child→parent）、tag_products 覆盖计数）+ knowledge 三 section 防御式并入（未知产品/API 引用一律丢弃，构建为运行时前最后防线）；②LLM 构建期语义抽取（`apie/entity_extract.py`，`--llm` 且 `ENTITY_LLM_BASE_URL/API_KEY/MODEL` 齐备时）：产品级别名+语义关联（roster 指纹控制）、API 级口语查询关键词（产品出题、目标约束在真实 API 清单内、每 API ≤5——密集关键词拉长 BM25 字段恶化长度归一化、域约束禁用他产品专属词、覆盖生命周期动词短语、sha256 指纹增量仅重抽变化产品，--llm-force 全量重抽）、三道闸（严格 JSON 解析容忍围栏 / 交叉验证长度·存在性·去重·封顶 / 丢弃台账 WARNING）、`merge_knowledge` 世代合并（update 提供 key 才替换 llm 代；**curated 永不覆盖**；批失败跳过且指纹不更新，可恢复重跑）、增量落盘（`on_update` 每产品完成即原子写 knowledge，长任务中断凭指纹台账续跑只补缺口；默认批 20 产品/80 API，`--batch-products/--batch-apis` 可调）。产物 `data/graph/entity-index.json` + 提交副本 `configs/entity-index.json`（wheel bundle 唯一运行时依赖）。**运行时 BM25 引擎（S16，2026-09 起，tantivy 重构）**：`mcp_openapi/entity_search.py` TantivyEngine（deep module，唯一 tantivy import 点）——RAM 索引（parse 时一次性构建 ~600ms/17.8k docs，规避索引格式跨版本磁盘兼容）、手动 token 化（doc/query 同规则：name 字段整名小写+驼峰切分、ASCII 不做 2-gram；summary/tags 整段+CJK 2-gram；kw_exact 关键词整值、kw_grams 关键词 2-gram）、字段 boost（FieldBoosts：name 3 / kw_exact 2.5 / summary/tags/kw_grams 1）、查询结构=字段内按术语分组（单术语 token should-sum）×术语间 DisMax（tie_breaker=1 等价 sum；压 kw 密集产品的碎片和放大）×字段间加权 sum、单产品补召回 `search_product`（product 字段 MUST 过滤 + 以主查询全局 top 归一化，保证分数跨产品可比——身份活跃但落出命中池的产品在 BM25 里有话语权）。`mcp_openapi/entity_graph.py` EntityGraph（S16b facade，公开接口不变）：`_terms` 切分（空白/标点 + ASCII↔CJK 边界，连续 CJK 不分词）→ 身份信号 Python 精确子串（alias 四档每产品每查询一次：嵌入用户原句 18 > 整别名 18 > 术语∈别名 8（纯 ASCII 术语须 ≥3 字符，防 ip ⊆ 公网ip 弱身份）> gram 碎片∈别名 6（防 LLM 多别名互嵌碎片逐条累加——实测 专属云主机/独享主机）/ product_short 精确 12 / name 5 / category 1 / 判别 tag bonus 2（coverage ≤3，严格子串判定——gram 重叠≠包含））+ BM25 池内最优文档 ×12 + 广度先验（平方 log API 数 ×2.5，BM25 无「产品显著性」概念的统计先验，零证据产品不计）→ twin 归并（分数取 max、主产品+related twin 边）→ `matched_via` 证据（优先级排序去重 cap 3：product < alias exact < kw exact < tag < kw contains < name < alias contains < alias gram；kw/tag 证据为严格子串语义，gram 级弱命中只贡献 BM25 分数不出证据）。**金评集门禁** `tests/fixtures/entity_eval.json`（45 例人工标注独立真值：top1/top3/family 三档，tests/eval_runner.py 执行；分数不再手算——mini fixture 断言行为/证据/机制参数，排序质量由金评集固化）。命中池 _DOC_POOL=64（身份产品不受池约束）；LLM 输出只存在于构建期产物与 knowledge 文件（带 provenance），运行时检索零 LLM/零网络；`--entity-index` 三分支（None→bundle 缺失静默空 / off→显式禁用 / 路径→fail-fast，裸文件名经 resolve_config_arg）；search_apis 为元数据工具：gate restrict 经 allowed 机制参数排名前过滤（total/truncated 与过滤后集合一致）、**废弃治理同 list_apis**（--deprecated-index + --deprecated-mode 共用——hide 排名前排除，annotate 条目标注 deprecated+replacement，off/未配置索引恒现状；发现面收窄 ≠ 详情拒绝，get_api/execute_api 恒可用）、无 policy 语义、无 hints 注入、产物为构建期快照（与 hints 同性质，非实时）。
- 审计 NDJSON（`HUAWEICLOUD_MCP_AUDIT_FILE` / `--audit-file`）：service 层 8 个 openapi 工具每次调用经 `AuditSink.record` 追加一行 `{"ts", "tool", "input", "ok"}`（ts ISO8601 UTC 由 sink 注入；input 为显式入参快照不含默认值；ok 缺省视为 true；异常路径记 ok=False 后原样抛出）；**会话归因（ADR-0003，S20）**：`_audit_write` 写事件单点从 ambient contextvar（common.sessions）读会话键，键非 None（HTTP 档）时事件顶层附加 `session` 字段——多会话部署的「谁授予/谁执行」归因，input 快照纯净性与 `build_audit_event` 契约形状（tool/input/ok）不动，verifier 加性键零回归；stdio 恒 None 恒不加键（NDJSON 逐字节红线）；best-effort 永不抛出（写失败 WARNING）；事件 payload 契约由 `mcp_openapi.service.build_audit_event` 定义并经 `scorer.event_to_toolcall` 成为 harbor verifier 的 trace 输入源；未配置 sink 时零开销跳过。
- Streamable HTTP 传输与会话隔离（ADR-0003，2026-09 起）：`--transport stdio|http`（`streamable-http` 别名归一，非法值启动 fail-fast——有意区别于 parse_modes 宽容回退）+ 监听三元组 `--http-host/--http-port/--http-path`（默认 127.0.0.1:8000/mcp；loopback 自动启用 SDK DNS rebinding 防护，非 loopback 绑定 WARNING 点名 AK/SK 暴露面；容器开箱由镜像层 ENV 置 0.0.0.0）。**服务分派归 cli**（`run_transport` internal seam：stdio `app.run("stdio")` 逐字节现状 / http `app.run("streamable-http", host, port, path)`）——不自建 TransportPort（SDK run 即 port 的双 adapter，自建过不了删除测试）、不建 build_asgi/serve_http 模块（`streamable_http_app()`/`custom_route("/healthz")` 已是机制）。**会话键控**：PolicyStore `_overlay` 按会话键分桶（None 桶=历史命名空间，stdio 逐字节回归由结构保证）、`session_fn` 构造注入（safety 零内部依赖不破）、会话身份经 `SessionScopeMiddleware`（SDK ServerMiddleware——消息跨 task 派发使 ASGI 中间件 contextvar 不可达，dispatch 所在会话 task 内的 ServerMiddleware 是唯一咽喉点）绑 ambient contextvar，`build_app` 单点配对（middleware 注册 + session_fn 注入同处装配）；互斥桶（评估点恰一活跃桶）、跨会话不可见（含 list_rules）、once 限授予会话桶内焚毁。**build_app 原则界定**：serving 与返回类型 transport-agnostic（永不 run/恒返回 MCPServer），augmentation 允许 transport-aware（strict_sessions 置位、/healthz 注册、instructions 传输说明段——stdio 文本逐字节不变）。**明确不做**：SSE、stateless_http（无会话身份使 session 档授权无所附着，旋钮不暴露）、多 worker（进程内存状态 fork 不可预测）、v1 内置 HTTP 认证/TLS（定位本机/可信网段，跨主机须反代）。**F1 残余风险（已记录）**：嵌入方以 stdio 声明的 build_app 产物自行组合 `streamable_http_app()`——携带身份的会话仍隔离，无身份请求的 session 档授予落共享 None 桶；缓解=配方强制「组合 ASGI ⇒ args 声明 `--transport http`」。嵌入配方：`build_app(...).streamable_http_app()` 交 uvicorn/gunicorn 驱动（lifespan 已接线 session manager，勿二次 Mount 丢 lifespan）。discover SessionManager 保持进程共享（授权接缝是 policy 非连接所有权，记录为已知边界）；HTTP 会话无 SDK 级 idle 回收（`streamable_http_app()` 不透传 session_idle_timeout，overlay GC 兜底）。
